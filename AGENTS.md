The `<project-knowledge-harness>` prefix refers to the installed skill directory.
...
The bundled `<project-knowledge-harness>/scripts/todo-kanban.sh` validates the format. Run it
(`<project-knowledge-harness>/scripts/todo-kanban.sh --validate-only TODO.md`) after editing so syntax
drift is caught immediately.

#### Three ways to add a TODO entry (preferred order)

1. **Structured CLI — `<project-knowledge-harness>/scripts/add-todo.sh`** (default):

   ```
   <project-knowledge-harness>/scripts/add-todo.sh --priority P3 --effort M \
...
2. **Quick capture — `backlog/inbox.md`** (when priority/effort unclear):
   ...
   `<project-knowledge-harness>/scripts/sweep-inbox.sh`. It prompts for the missing fields per loose
...
3. **Direct edit of `TODO.md`** — fine if the format is fresh; run
   `<project-knowledge-harness>/scripts/todo-kanban.sh --validate-only` afterwards.
...
1. Run `<project-knowledge-harness>/scripts/promote-todo.sh --title "<substring>" --summary "<what shipped>"`
...
`backlog/` is excluded from source archives by `.gitattributes` and Go module ZIPs by the nested `go.mod`; it
...
`pitfalls/` is excluded from source archives by `.gitattributes` and Go module ZIPs by the nested `go.mod` and
