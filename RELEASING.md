...
- Archives: `exp-cli_<version-without-v>_<os>_<arch>.tar.gz`.
- Each archive contains the flat `exp` executable, `LICENSE`, and
  `completions/exp.bash` / `completions/exp.zsh`.
- `checksums.txt` names exactly those four archives.
- The linker injects the Git tag into `github.com/daviddwlee84/exp-cli/internal/cli.Version`.
...
python3 scripts/release.py check --project exp-cli --binary exp --smoke
```
